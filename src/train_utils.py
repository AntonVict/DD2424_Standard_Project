...
BATCH_SIZE = 32
EPOCHS = 1
SUBSET_RATIO = 0.1  # Use only 10% of the data for faster training
...
def set_parameter_requires_grad(model, num_blocks, strategy):
    ...
    if strategy == 'simultaneous':
        ...
        print(f"Unfrozen blocks in layer4: {sorted(blocks_unfrozen)}")

def accuracy(outputs, labels):
    preds = torch.argmax(outputs, dim=1)
    return (preds == labels).float().mean().item()

def train_model(data_dir, train_file, strategy='gradual', num_blocks=3, epochs=EPOCHS, batch_size=BATCH_SIZE, subset_ratio=SUBSET_RATIO):
    # Device selection
    ...
    set_parameter_requires_grad(model, num_blocks, strategy)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (imgs, labels) in enumerate(tqdm(train_dl)):
            ...
            if (batch_idx + 1) % 50 == 0:
                ...
                val_loss /= len(val_ds)
                val_acc /= len(val_ds)
                print(f"Batch {batch_idx+1} | Val Loss: {val_loss:.4f} | Val Acc: {val_acc:.4f}")
                model.train()
...
def plot_training_results(train_losses, train_accs, val_losses, val_accs, save_dir):
